...
const RequestsScreen: React.FC = () => {
  ...
  const confirmMessage = selectedIds.size > 0 && selectedIds.size < completed.length
    ? t('requests.deleteConfirmMultiple', { count: selectedIds.size })
    : t('requests.deleteConfirmAll');
      ...
      <ScreenWrapper>
        <ScrollView contentContainerStyle={styles.container}>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtitle}>{t('requests.noRequestsSubtext')}</Text>
            <TouchableOpacity style={styles.ctaButton} onPress={() => navigation.navigate("Identify")}>
              <MaterialCommunityIcons name="camera" size={20} color="#fff" />
              <Text style={styles.ctaText}>{t('requests.identifyWaste')}</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </ScreenWrapper>
  ...
  return (
    <ScreenWrapper>
      <View style={styles.pageHeader}>
        <View>
          {selectMode ? (
            <Text style={styles.title}>{t('requests.selected', { count: selectedIds.size })}</Text>
          ) : (
            <>
              <Text style={styles.title}>{t('requests.title')}</Text>
              <Text style={styles.subtitle}>{t('requests.totalRequests', { count: requests.length })}</Text>
            </>
          )}
        </View>
          ...
          {selectMode ? (
            <>
              <TouchableOpacity onPress={allSelected ? deselectAll : selectAll} style={styles.headerTextBtn}>
                <Text style={styles.headerTextBtnLabel}>{allSelected ? t('requests.deselectAll') : t('requests.selectAll')}</Text>
              </TouchableOpacity>
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        {active.length > 0 && (
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <Text style={styles.sectionTitle}>{t('requests.active')}</Text>
            ...
            {active.map((item) => {
              ...
              const partnerDisplayName = partnerId ? (partnerNames.get(partnerId) || t('requests.recyclerPartner')) : t('requests.pendingAssignment');
                  ...
                  <Card style={styles.requestCard}>
                    <View style={styles.cardHeader}>
                      ...
                      <View style={styles.cardContent}>
                        <Text style={styles.wasteTitle}>{t(`wasteCategory.${wasteType.toLowerCase()}`) || wasteType} {t('requests.waste')}</Text>
                    ...
                    <View style={styles.cardDetails}>
                      <View style={styles.detailItem}>
                        ...
                        <Text style={styles.detailText}>{t('requests.items', { count: quantity })}</Text>
                      </View>
                      <View style={styles.detailItem}>
                      ...
                      <View style={styles.scheduledBanner}>
                        ...
                        <Text style={styles.scheduledText}>
                          {(() => {
                            ...
                            return t('requests.scheduled', { datetime: `${date.toLocaleDateString()} ${date.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}` });
                          })()}
                        </Text>
                      </View>
                    )}
                    <View style={styles.statusSection}>
                      <View style={styles.statusBadge}>
                        ...
                        <Text style={styles.statusText}>{t(`status.${item.status}`) || item.status}</Text>
                      </View>
        ...
        {completed.length > 0 && (
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <Text style={styles.sectionTitle}>{t('requests.completedSection')}</Text>
              <View style={[styles.countBadge, { backgroundColor: '#ecfdf5' }]}>
                <Text style={[styles.countText, { color: '#10b981' }]}>{completed.length}</Text>
              </View>
              {!selectMode && (
                <Text style={styles.selectHint}>{t('requests.longPressHint')}</Text>
              )}
            </View>
            {completed.map((c) => {
              const completedWasteType = (c as any).type || (c as any).wasteType || 'Unknown';
              const completedPartnerId = (c as any).partnerId;
              const completedPartnerName = completedPartnerId ? (partnerNames.get(completedPartnerId) || t('requests.recyclerPartner')) : t('requests.recyclerPartner');
                ...
                >
                  <Card style={[styles.requestCard, styles.completedCard, isSelected && styles.cardSelected]}>
                    <View style={styles.cardHeader}>
                      ...
                      <View style={styles.cardContent}>
                        <Text style={styles.wasteTitle}>{t(`wasteCategory.${completedWasteType.toLowerCase()}`) || completedWasteType} {t('requests.waste')}</Text>
                    ...
                    <View style={styles.completedFooter}>
                      <View style={styles.detailItem}>
                        ...
                        <Text style={styles.detailTextSmall}>{t('requests.items', { count: quantity })}</Text>
                      </View>
                      <View style={styles.detailItem}>
        ...
        {totalItems > 0 && !selectMode && (
          <LinearGradient colors={['#ecfdf5', '#d1fae5']} start={[0, 0]} end={[1, 1]} style={styles.impactCard}>
            ...
            <View style={{ flex: 1, marginLeft: 16 }}>
              <Text style={styles.impactTitle}>{t('requests.yourImpact')}</Text>
              <Text style={styles.impactValue}>{t('requests.itemsRecycled', { count: totalItems })}</Text>
              <Text style={styles.impactSubtext}>{t('requests.keepUpWork')}</Text>
            </View>
          </LinearGradient>
        )}
      ...
      <Modal visible={showConfirm} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <Text style={styles.modalTitle}>{t('requests.deleteRequests')}</Text>
            <Text style={styles.modalMessage}>{confirmMessage} {t('requests.cannotUndo')}</Text>
